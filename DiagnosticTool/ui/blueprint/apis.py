...
def setTrigger(taskId):
    ...
    if run_id and time:
        ...
        try:
            result = requests.post(URL, headers = {'Accept': 'application/json', 'Content-Type': 'application/x-www-form-urlencoded'}, 
            data = json.dumps({'run_id' : run_id,'execution_date': execution_date, 'conf': conf}))
